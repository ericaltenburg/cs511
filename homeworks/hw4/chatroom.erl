...
do_propegate_message(State, Ref, ClientPID, Message) ->
   	...
   	D = maps:keys(maps:remove(ClientPID, State#chat_st.registrations)),
   	lists:foreach(
   		fun(X) ->
   			X!{request, self(), Ref, 
   				{incoming_msg, Nick, State#chat_st.name, Message}
			}
		end,
		D),
   	#chat_st {
   		...
   		history = lists:append(State#chat_st.history, [{Nick, Message}])
   	}.